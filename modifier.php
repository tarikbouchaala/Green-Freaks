...
<body>
    ...
    try {
        $req = "SELECT * FROM greenworks WHERE id_User_Green =" . $_GET['id'] . " AND idGreen='" . $_GET['idGreen'] . "'";
    ...
    <div class="container">
        <form action="" method="POST" enctype="multipart/form-data">
            <input type="text" name="title" id="title" placeholder="Titre du GreenWork" value="<?php if (isset($_GET['id']) && !(empty($_GET['id'])) && isset($_GET['idGreen']) && !(empty($_GET['idGreen']))) echo $ligne['title'] ?>">
            <textarea name="ingred" id="ingred" maxlength="250" placeholder="Ingrédients"><?php if (isset($_GET['id']) && !(empty($_GET['id'])) && isset($_GET['idGreen']) && !(empty($_GET['idGreen']))) echo $ligne['ingredients'] ?></textarea>
            <textarea name="etapes" id="etapes" maxlength="1000" placeholder="Etapes et démarches de réalisation"><?php if (isset($_GET['id']) && !(empty($_GET['id'])) && isset($_GET['idGreen']) && !(empty($_GET['idGreen']))) echo $ligne['etapes'] ?></textarea>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($title) && !(empty($title)) && isset($ingred) && !(empty($ingred)) && isset($etapes) && !(empty($etapes))) {
                ...
                if (count($imgs['name']) > 6) {
                    ...
                    try {
                        $req = $db->prepare("UPDATE greenworks SET title=:title,ingredients=:ingredients,etapes=:etapes,image=:img WHERE idGreen=" . $ligne['idGreen']);
                        $req->execute(
                            array(
                                "title" => $title,
                                "ingredients" => $ingred,
                        ...
                        echo "Insertion des donnes a échoué" . $errrr->getMessage();
                    }
                }
            } else {
